...
for row in orders:
    ...
    success_c         = confirm_requests - price_changed - expired - other_err_c
    success_c         = max(1, success_c)
    success_rate_c    = round(success_c / confirm_requests * 100, 2)
...
print(f"  查价请求：{total_search:>8,}")
print(f"  验价请求：{total_confirm:>8,}  (查价转化率 {total_confirm/total_search*100:.1f}%)")
print(f"  订单数：  {total_booking:>8,}  (验价转化率 {total_booking/total_confirm*100:.1f}%)")
